...
export default function PreferenceResultsPage() {
  ...
  const { result, reset } = usePreferenceStore();

  if (!result) {
    return (
      <AppLayout>
        <Text style={styles.empty}>No results found. Please complete the questionnaire.</Text>
        <Link href={ROUTES.preferenceMatching}>
          <Text style={styles.link}>← Start matching</Text>
        </Link>
      </AppLayout>
    );
  }
      ...
      <View style={styles.matchGrid}>
        {result.top_matches.map((match, idx) => (
          <Card key={match.candidate_id} style={styles.matchCard}>
            ...
            <Link href={ROUTES.candidateDetail(match.candidate_id)} style={styles.viewLink}>
